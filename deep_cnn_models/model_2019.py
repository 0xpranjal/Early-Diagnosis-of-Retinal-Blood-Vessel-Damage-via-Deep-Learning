...
MODEL_ARCH="resnet50" # "resnet50" or "xception" or "inception_v3"
EPOCHS = 20
IMG_SIZE = 256
BATCH_SIZE = 32
VAL_BATCH_SIZE = 32
...
ITER_VISUALS =500
WEIGHTS_DIR = MODEL_ARCH + "_weights"
...
class retinopathy2015(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img = cv2.imread(f"../resized_train_19/{file_name}.jpg")
...
def valid_fn(epoch, model, criterion, val_loader, device, scheduler):
    ...
    print(roc_auc)
    accuracy= accuracy_score(np.argmax(TARGETS,axis = 1),np.argmax(PREDS,axis = 1))
    print(f"Accuracy : {accuracy}")
    cohen  = cohen_kappa_score(np.argmax(PREDS,axis =1),np.argmax( TARGETS,axis = 1), weights='quadratic')
    print(f"Cohen : {cohen}")
    if scheduler is not None and SCHEDULER_UPDATE == 'epoch':
        scheduler.step(losses.avg)
...
def engine(device,df,fold):
    ...
    model = Model(MODEL_ARCH, num_classes=5, pretrained=True)
    model.load_state_dict(torch.load("resnet50_weights/resnet50_256_epoch_19.pth"))
    model.to(device)
    ...
    for epoch in range(START_EPOCH, EPOCHS):
        ...
        with open(f'GPU_{MODEL_ARCH}_{IMG_SIZE}.txt', 'a') as appender:
            appender.write(content + '\n')  # avg_train_accuracy: {avg_accuracy:.4f}

        with open(f'{MODEL_ARCH}_{IMG_SIZE}_ROC_LOSS.json',"w") as out:
            json.dump(out_dict,out)

        torch.save(model.state_dict(), f'{WEIGHTS_DIR}/{MODEL_ARCH}_{IMG_SIZE}_epoch_{(epoch + 1)}.pth')
        torch.cuda.empty_cache()
    """

    avg_val_loss,roc_auc_score = valid_fn(START_EPOCH, model, val_criterion, val_loader, device, scheduler)
    return loss

if __name__ == '__main__':
    df_2015 = pd.read_csv("../labels/trainLabels19.csv")
    df_2015.columns = ['image','level']
